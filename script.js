...
function editCurrentProfile(){const p=profileStore.profiles[activeProfileId];modal(`<h2>⚙️ Customize Profile</h2><label>Profile name</label><input id="profileName" class="form-input" value="${esc(p.name)}"><label>Avatar / emoji</label><input id="profileAvatar" class="form-input" value="${esc(p.avatar||"🧶")}" maxlength="4"><label>Theme</label><select id="profileTheme" class="form-input">${Object.entries(THEMES).map(([k,t])=>`<option value="${k}" ${p.theme===k?"selected":""}>${t.emoji} ${t.label}</option>`).join("")}</select><div class="form-actions"><button class="mini-btn" onclick="openProfile()">Cancel</button><button class="primary-btn" onclick="saveCurrentProfile()">Save Profile ✓</button></div>`)}
async function saveCurrentProfile(){const p=profileStore.profiles[activeProfileId];p.name=document.getElementById("profileName").value.trim()||"Crochet Journal";p.avatar=document.getElementById("profileAvatar").value.trim()||"🧶";p.theme=document.getElementById("profileTheme").value;profileStore.activeId=activeProfileId;localStorage.setItem(PROFILE_KEY,JSON.stringify(profileStore));applyTheme();closeModal();await cloudSave();toast("Profile updated and synced ✓")}
...
function renderPatterns(){
 ...
 const list=data.patterns.filter(p=>filter==="All"||p.category===filter);
