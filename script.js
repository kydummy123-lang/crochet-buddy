...
function selectProfileImage(input,targetId="profileImageValue"){const f=input.files?.[0];if(!f)return;pendingImages.profileImage=f;const hidden=document.getElementById(targetId);if(hidden)hidden.value="";const preview=document.getElementById(targetId+"Preview");if(preview){const url=URL.createObjectURL(f);preview.src=url;preview.style.display="block";preview.onload=()=>URL.revokeObjectURL(url);}}
function newProfile(){pendingImages.profileImage=null;modal(`<h2>＋ New Crochet Profile</h2><label>Profile name</label><input id="profileName" class="form-input" placeholder="e.g. Mom's Crochet Journal"><label>Profile picture</label><input id="newProfileImage" type="file" accept="image/*" class="form-input" onchange="selectProfileImage(this,'newProfileImageValue')"><input id="newProfileImageValue" type="hidden" value=""><img id="newProfileImageValuePreview" class="profile-edit-avatar" style="display:none;object-fit:cover" alt="Profile picture preview"><small class="meta">Choose a photo from your phone or computer. It will be saved to your cloud account.</small><label>Choose an avatar</label><input id="profileAvatar" class="form-input" value="🧶" maxlength="4"><label>Theme</label><select id="profileTheme" class="form-input">${Object.entries(THEMES).map(([k,t])=>`<option value="${k}">${t.emoji} ${t.label}</option>`).join("")}</select><div class="form-actions"><button class="mini-btn" onclick="openProfile()">Cancel</button><button class="primary-btn" onclick="createProfile()">Create Profile ✓</button></div>`)}
async function createProfile(){const name=document.getElementById("profileName").value.trim()||"Crochet Journal";const avatar=document.getElementById("profileAvatar").value.trim()||"🧶";const theme=document.getElementById("profileTheme").value;let avatarImage="";if(pendingImages.profileImage){if(!cloudUser){alert("Please sign in to save a profile picture to cloud storage.");return;}try{setSyncStatus("☁️ Uploading profile picture…","saving");avatarImage=await uploadImageFile(pendingImages.profileImage,"profile");delete pendingImages.profileImage;}catch(e){alert("The profile picture could not be uploaded. Please try again.");return;}}const id="profile-"+Date.now();profileStore.profiles[id]={name,avatar,theme,avatarImage,data:normalizeData({categories:[...defaultCategories],patterns:[],projects:[],yarn:[],learnedLessons:[],myLearnings:[]})};profileStore.activeId=id;activeProfileId=id;data=profileStore.profiles[id].data;localStorage.setItem(PROFILE_KEY,JSON.stringify(profileStore));applyTheme();closeModal();renderAll();await cloudSave();toast("Profile created and synced ✓")}
...
:`<div class="form-actions"><button class="mini-btn" onclick="closeModal()">Close</button><button class="primary-btn" onclick="editPattern('${p.id}')">✏️ Edit Pattern</button><button class="primary-btn" onclick="printPatternPDF('${p.id}')">📄 Save as PDF</button><button class="primary-btn" onclick="exportPatternWord('${p.id}')">📝 Save as Word</button></div>`}
}
